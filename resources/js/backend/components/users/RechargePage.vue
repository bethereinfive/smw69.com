...
<div>
<p class="bg-warning mx-3 ps-4 py-2 rounded-top-5 pt-3" style="margin-bottom: 9px;;font-size: 12px;">অর্ডার আইডি : {{ form.orderid }}</p>
<div>
  <p class="bg-warning fs-1 fw-medium mx-3 pt-2 py-5 rounded-bottom-5 text-center">{{ form.amount }} TK</p>
</div>
...
<div class="mx-4 ">
  ...
  <select class="form-select py-2 rounded-5 onepaynumberInput" aria-label="Default select example"  v-model="form.method" required>
    ...
    <option v-for="pay in row" :key="'pay' + pay.id" :value="pay.id">{{ pay.name }}</option>
  </select>
